...
def close_last():
    ...
    if order_type == OrderType.SHORT:
        ...
        if result == 0:  # Assuming 0 indicates a successful close
            order_type = OrderType.NONE
...
def make_trade_decision(sma5, sma8, sma13, klines, close_prices):
    ...
    if ordered_price is not None and order_type is not OrderType.NONE :
        profit = (close_prices[-1] - ordered_price) / ordered_price  # Calculate profit as a percentage
        logger.info(f"Calculated profit = {profit}")
        # Check if the trade needs to be closed based on TP or SL
        if (order_type==OrderType.LONG and profit >= TP)  or (order_type==OrderType.SHORT and profit <= SL):
            logger.info(f"Closing position due to {'Take Profit' if profit >= TP else 'Stop Loss'} condition.")
            close_last()
    ...
    if abs(sma5 - sma8) / sma8 > THRESHOLD_PERCENTAGE and abs(sma8 - sma13) / sma13 > THRESHOLD_PERCENTAGE:
        if sma5 > sma8 > sma13 and atr > 0.005:  # Buy signal criteria
            ...
            if count_of_long == 1:  # Ensure only one long trade at a time
                if close_prices[-1] > sma5:  # Confirmation: Last close price should be above SMA5
                    last_order_id, order_type = open_long(symbol=SYMBOL, quantity=0.015)
                    logger.info(f"LONG opened at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} in this price : {close_prices[-1]}")
                    count_of_long += 1
                    ordered_price = close_prices[-1]  # Store the entry price for profit calculation
            ...
            if count_of_short == 1:  # Ensure only one short trade at a time
                if close_prices[-1] < sma5:  # Confirmation: Last close price should be below SMA5
                    last_order_id, order_type = open_short(symbol=SYMBOL, quantity=0.015)
                    logger.info(f"SHORT opened at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} in this price : {close_prices[-1]}")
                    count_of_short += 1
                    ordered_price = close_prices[-1]  # Store the entry price for profit calculation
        else:
